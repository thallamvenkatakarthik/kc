...
def detect_aruco(image):
    ...
    return center_aruco_list, distance_from_rgb_list, angle_aruco_list, width_aruco_list, valid_ids, rvecs_out, tvecs_out
...
class aruco_tf(Node):
    ...
    def process_image(self):
        ...
        # Camera offsets relative to robot base
        ...
        # Use one constant depth for all detected fruits
        ...
        for cnt in bad_fruit_contours:
            ...
            # Crop ROI and find green top area
            roi = img[y:y + h, x:x + w]
            ...
            # Use shared depth value for all fruits
        ...
        if ids is not None and len(ids) > 0 and tvecs is not None:
            for idx, marker_id in enumerate(ids):
                ...
                base_x = CAMERA_OFFSET_X + Z_cam
                base_y = CAMERA_OFFSET_Y + X_cam
                base_z = CAMERA_OFFSET_Z - Y_cam
                ...
                t_obj.transform.rotation.x = float(combined[0])
                t_obj.transform.rotation.y = float(combined[1])
                t_obj.transform.rotation.z = float(combined[2])
                t_obj.transform.rotation.w = float(combined[3])
